...
export default function LegalPage({ kind }) {
  const { legalName, legalEmail, siteName } = useBranding();

  useEffect(() => {
    track("page_view", { page: kind });
    document.title = `${kind} · ${siteName}`;
  }, [kind, siteName]);
  ...
  return (
    <>
      <SEO
        ...
        description={`${meta.title} for ${siteName} — last updated January 2026.`}
      ...
      <section className="py-16 bg-white">
        <article className="max-w-3xl mx-auto px-6 md:px-10 lg:px-12 space-y-5 text-[#0A0A0A] leading-relaxed">
          ...
          <p>{legalName} ("we", "us") operates an assistance marketplace connecting commuters with verified Sathis. This {meta.title.toLowerCase()} governs your use of our website, mobile app, and APIs.</p>
          ...
          <p>{kind === "privacy" ? "Firebase (auth, data), Cashfree (payments), Twilio/Exotel (masked calling), Sentry (error logs). Each operates under their own privacy policy and is processed under DPA contracts." : kind === "terms" ? `${siteName} is an intermediary. We do not guarantee NHAI / bank resolution outcomes; we guarantee that a verified Sathi will attempt resolution within agreed SLAs.` : "Refunds initiated within 24 hours of dispute close. Credit reflects in source account within 5–7 business days via Cashfree."}</p>

          <h2 className="font-display font-black text-2xl pt-4">4. Contact</h2>
          <p>Reach <a className="text-[#FF6B00] underline" href={`mailto:${legalEmail}`}>{legalEmail}</a> for any clarifications, data-deletion requests, or grievance redressal under the IT Rules 2021.</p>
        </article>
      </section>
    </>
  );
