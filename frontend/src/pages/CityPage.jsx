...
import { MapPin, Users, AlertCircle, ChevronRight, Clock } from "lucide-react";
...
export default function CityPage() {
  ...
  const faqs = [
    { q: `How many FASTag Sathis are in ${city.name}?`, a: `ApnaFastag currently has ${city.sathiCount} verified Sathis across ${city.plazaCount} toll plazas in and around ${city.name}. Coverage is expanding.` },
  ...
  return (
    <>
      <SEO
        title={`FASTag help in ${city.name} — Sathis at ${city.plazaCount} toll plazas`}
        description={`${city.sathiCount} verified Sathis across ${city.plazaCount} toll plazas in ${city.name}. FASTag dispute, blacklist fix, KYC update — resolved on-spot in under 8 minutes.`}
        path={`/city/${city.slug}`}
      ...
      <PageHero
        ...
        sub={`${city.sathiCount} verified Sathis across ${city.plazaCount} toll plazas in ${city.name}. Stuck at a toll? Your Sathi arrives in 90 seconds.`}
        breadcrumb={[
      ...
      <section className="py-10 bg-[#F8F9FA] border-b-2 border-[#0A0A0A]">
        <div className="max-w-5xl mx-auto px-6 md:px-10 lg:px-12 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            { label: "Active Sathis", value: city.sathiCount },
            { label: "Toll plazas covered", value: city.plazaCount },
